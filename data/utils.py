...
def decode_seg_map_sequence(img, label_masks, dataset='pascal', plot=False):
    rgb_masks = []
    for label_mask in label_masks:
        rgb_mask = decode_segmap(img, label_mask, dataset, plot)
        rgb_masks.append(rgb_mask)
    # rgb_masks = torch.from_numpy(np.array(rgb_masks).transpose([0, 3, 1, 2]))
    return rgb_masks


def decode_segmap(img, label_mask, dataset, plot=False):
